...
purrr::map_dfr(1:4,make_perf_table) |>
  dplyr::mutate(
    across(matches("bias|ese|rmse"), ~formatC(.x, digits = 3, format = "f"))
  ) |>
...
purrr::map_dfr(1:4,make_perf_table) |>
  dplyr::mutate(
    across(matches("bias|ese|rmse"), ~formatC(.x, digits = 4, format = "f"))
  ) |>
...
ggplot(fig, mapping = aes(x = analyze, y = rr, group = shape_lbl, shape = shape_lbl)) +
  geom_hline(aes(yintercept = truth), linetype = "dashed", color = "red", size = 1) +
  geom_point(size = 3, position = position_dodge(width = 0.5)) +
  scale_y_continuous(limits = c(0.5, 0.75), breaks = seq(0.5, 0.75, by = 0.05)) +
  labs(x = "Analyzed population", y = "Risk ratio", shape = "Target population") +
  theme_classic() +
